...
SparseSolvのBDDCはNGSolveの組込みBDDCと**同じアルゴリズム**を実装しており、
反復回数・計算時間ともに同等の性能を示す。

| 問題 | SparseSolv BDDC | NGSolve BDDC |
|------|----------------|--------------|
| H1 Poisson (order=3) | 2反復 | 19反復 |
| VectorH1弾性体 (order=2) | 2反復 | 14反復 |
| HCurl curl-curl (order=2) | 30反復 | 29反復 |
| HCurl渦電流 複素数 (order=2) | 16反復 | 17反復 |

注: 反復回数の差異はSchur補体の計算精度と重み付けの実装差による。
H1/VectorH1でSparseSolvが2反復と極めて少ないのは、
要素単位の密LU逆行列が厳密であるため。
